...
export async function spamInstagramDms(
  ...
  //click next
  await page.waitForSelector('button._acan._aiit._acao._aija._acas._acav._aiiv._aj1-');
...
export async function spamInstagramCommentsIfPublic(  
  ...
  if ((await page.$$('h2._aa_u')).length!== 0 && await page.$eval('h2._aa_u', (h2) => h2.textContent) !== null) {
    await browser.close();
    return 'account is private';
  } else {
    var posts = await (await page.$$('div._aabd._aa8k._aanf')).map(async (post) => await post.$eval('a', (a) => a.href));
    ...
    for (var i = 0; i < posts.length; i++) {
      ...
      await page.type('textarea[placeholder="Add a comment…"]', message, {delay: 20});
      await page.keyboard.press('Enter');
